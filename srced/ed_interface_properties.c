...
static void	draw_cbend_n_nextmap(t_data *d, SDL_Surface **prop, int x, int y)
{
	...
	copy_surface_to_surface(prop[16], d->sdl.screen, (int[2]){x, y + 3}, d);
	...
	copy_surface_to_surface(((d->selected_sector != -1 &&
			...
			d->sdl.screen, (int[2]){d->interface.cbox_end_p.x, y}, d);
	x = W - PROPERTIES_LIMIT + 8;
	y = d->interface.cbox_end_p.y + 40;
	copy_surface_to_surface(prop[17], d->sdl.screen, (int[2]){x, y}, d);
														...
														int x, int y)
{
	...
	copy_surface_to_surface(prop[8], d->sdl.screen, (int[2]){x, y}, d);
	copy_surface_to_surface(prop[9], d->sdl.screen, (int[2]){x + 32, y}, d);
	y += prop[1]->h * 0.5 + 17;
	...
	copy_surface_to_surface(prop[10], d->sdl.screen, (int[2]){x, y - 33}, d);
	copy_surface_to_surface(prop[11], d->sdl.screen, (int[2]){x, y}, d);
	d->interface.cbox_skybox_p = (t_vec2){x + prop[11]->w + 15, y};
	copy_surface_to_surface(((d->selected_sector != -1 &&
			d->sectors[d->selected_sector].outdoor) ? prop[14] : prop[13]),
			d->sdl.screen, (int[2]){d->interface.cbox_skybox_p.x, y}, d);
	y = d->interface.tex_select[2].y + 74;
	copy_surface_to_surface(prop[12], d->sdl.screen, (int[2]){x, y}, d);
	d->interface.cbox_door_p = (t_vec2){x + prop[12]->w + 15, y};
	copy_surface_to_surface((((d->selectedwall && d->selectedwall->is_door) ||
		(d->hl_wall && d->hl_wall->is_door)) ? prop[14] : prop[13]), d->sdl.screen,
...
static void	draw_selection_case(t_data *d, SDL_Surface **prop, int x, int y)
{
	...
	copy_surface_to_surface(prop[15], d->sdl.screen, (int[2]){x - 20, y}, d);
}
...
void		print_properties(t_data *d, SDL_Surface **properties, int x, int y)
{
	...
	copy_surface_to_surface(properties[0], d->sdl.screen, (int[2]){x, y}, d);
	y += properties[0]->h + 5;
	x = W - PROPERTIES_LIMIT + MARGIN;
	copy_surface_to_surface(properties[1], d->sdl.screen, (int[2]){x, y + 5}, d);
	y += properties[0]->h + properties[1]->h + 16 + 80 + MOVE_WALL_PROP;
	x = W - PROPERTIES_LIMIT + MARGIN;
	draw_separator(d, W - PROPERTIES_LIMIT, y - 5, 0x008800);
	copy_surface_to_surface(properties[2], d->sdl.screen, (int[2]){x, y + 5}, d);
	y += properties[2]->h + 5;
	x = W - PROPERTIES_LIMIT + MARGIN;
	copy_surface_to_surface(properties[3], d->sdl.screen, (int[2]){x, y + 32}, d);
	...
	copy_surface_to_surface(properties[20], d->sdl.screen, (int[2]){x, y - 2}, d);
	if (d->hl_wallnum != -1 && d->walls[d->hl_wallnum].is_transparent)
		copy_surface_to_surface(properties[14], d->sdl.screen,
