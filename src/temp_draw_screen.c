    ...
    *head = (struct item){player.sector, 0, WIDTH - 1};
    ...
    do
    {
        ...
        for (unsigned s = 0; s < sect->npoints; ++s)
        {
            ...
            float vx1 = sect->walls[s + 0].x - player.where.x, vy1 = sect->walls[s + 0].y - player.where.y;
            float vx2 = sect->walls[s + 1].x - player.where.x, vy2 = sect->walls[s + 1].y - player.where.y;
...
#define Yaw(y, z) (y + z * player.yaw)
            int
                y1a = HEIGHT / 2 - (int)(Yaw(yceil, tz1) * yscale1)
                                       y1b = HEIGHT / 2 - (int)(Yaw(yfloor, tz1) * yscale1);

            int
                y2a = HEIGHT / 2 - (int)(Yaw(yceil, tz2) * yscale2),
                y2b = HEIGHT / 2 - (int)(Yaw(yfloor, tz2) * yscale2);
            /* The same for the neighboring sector */
            int ny1a = HEIGHT / 2 - (int)(Yaw(nyceil, tz1) * yscale1),
                ny1b = HEIGHT / 2 - (int)(Yaw(nyfloor, tz1) * yscale1);
            int ny2a = HEIGHT / 2 - (int)(Yaw(nyceil, tz2) * yscale2),
                ny2b = HEIGHT / 2 - (int)(Yaw(nyfloor, tz2) * yscale2);
