...
void	proj_wall(t_data *d, t_projdata *p, t_frustum *fr, t_vec2f v[2])
{
	...
	p->y1a = get_ceildh(d, p->sector, v[0]) * -p->scale1 + offset;
	p->y2a = get_ceildh(d, p->sector, v[1]) * -p->scale2 + offset;
	p->y1b = get_floordh(d, p->sector, v[0]) * -p->scale1 + offset;
	p->y2b = get_floordh(d, p->sector, v[1]) * -p->scale2 + offset;
	...
	if (p->neighbor)
	{
		p->ny1a = get_ceildh(d, p->neighbor, v[0]) * -p->scale1 + offset;
		p->ny2a = get_ceildh(d, p->neighbor, v[1]) * -p->scale2 + offset;
		p->ny1b = get_floordh(d, p->neighbor, v[0]) * -p->scale1 + offset;
		p->ny2b = get_floordh(d, p->neighbor, v[1]) * -p->scale2 + offset;
	}
	proj_wall2(d, p, fr);
}
