...
void	proj_wall(t_env *d, t_proj_env *p, t_frustum *fr, t_vec2f v[2])
{
	double offset;
	...
	if (p->portal)
	{
		p->ny1a = cl_calc_vec_cal(d, p->portal, v[0]) * -p->scale1 + offset;
		p->ny2a = cl_calc_vec_cal(d, p->portal, v[1]) * -p->scale2 + offset;
		p->ny1b = fl_calc_vec_cal(d, p->portal, v[0]) * -p->scale1 + offset;
		p->ny2b = fl_calc_vec_cal(d, p->portal, v[1]) * -p->scale2 + offset;
	}
	proj_wall2(d, p, fr);
}
