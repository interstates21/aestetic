...
static int check_f(char *s, int i)
{
	...
	while (s[++i])
	{
		...
		if (s[i] == '-' && !ft_isspace(*(s + i - 1)))
			return (0);
		if (s[i] == '.' && (++point || !ft_isdigit(*(s + i - 1)) || !ft_isdigit(*(s + i + 1))))
			return (0);
	}
	return (1);
}
...
static int gut_check(char *s, t_scene *g, t_vt **v, int *n_v)
{
	if (!ft_strlen(s))
		return (1);
	if (s[0] == 'v')
...
int load_data(char *fname, t_scene *scene)
{
	...
	{
		if (!seq_check(&stage, line[0]) || !gut_check(line, scene, &vert, &n_vert))
		{
			free(line);
			return (0);
		}
