...
static int	ani(t_ed *e, SDL_Surface **s, int n_a)
{
	...
	i = -1;
	while (++i < n_a)
		if ((t += write(e->fd, &s[i]->w, sizeof(int))) < 0
		|| (t += write(e->fd, &s[i]->h, sizeof(int))) < 0
		|| (t += write(e->fd, s[i]->pixels, 4 * s[i]->w * s[i]->h)) < 0)
			return (1);
	return (0);
}
...
t = 0;
	tmp = e->n_sprites;
	if ((t += write(e->fd, &tmp, sizeof(int16_t))) < 0)
		print_err("saving n_sprites failed");
	i = -1;
	while (++i < e->n_sprites)
		if ((t += write(e->fd, &e->sprites[i].texture->w, sizeof(int))) < 0
		|| (t += write(e->fd, &e->sprites[i].texture->h, sizeof(int))) < 0
		|| (t += write(e->fd, e->sprites[i].texture->pixels, 4 *
		e->sprites[i].texture->w * e->sprites[i].texture->h)) < 0)
			print_err("saving sprite textures failed");
	printf("sprite_tex total written: %d\n", t);
}
...
void		write_mons_tex(t_ed *e)
{
	...
	printf("monster_tex total written: %d\n", t);
}
